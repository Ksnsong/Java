...
public class PlayChomp implements Runnable {
    ...
    public int afterClicked(MouseEvent e){
    	int clickX=e.getX();
    	int clickY=e.getY();
    	final int width = this.game.boxes.size();
...
class ChompDisplay extends JPanel {
    ...
    protected void paintComponent(Graphics graphics) {
        ...
        int width = this.game.boxes.size();
        int height = this.game.boxes.get(0).size();
        for (int i = 0; i < width; i++) {
        	for (int j = 0; j < height; j++) {
	            //graphics.drawRect(10 + (30 * i), 10 + (30 * j), 30, 30);
	            if (this.game.boxes.get(i).get(j) == 1) {
	                //graphics.drawString("C", 20 + (30 * i), 30 + (30 * j));
	            	graphics.drawOval(15 + (30 * i), 15 + (30 * j), 20, 20);
	            } else if (this.game.boxes.get(i).get(j) == 2) {
    ...
    public Dimension getPreferredSize() {
        int width = 30 + 31*this.game.boxes.size();
        int height = 55 + 31*this.game.boxes.get(0).size();
