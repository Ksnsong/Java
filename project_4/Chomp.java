/**
 * This class describes a game position of Chomp. 
...
public class Chomp {
	...
	/** Debugging output toggle. */
	private final boolean DEBUG = false;	
	
	/** The game board's boxes. */
        	...
        	for (int i = 0; i < width; i++) {						// and left-to-right, number the boxes... 
        		if (boxNumber == currentBox) {						// and if the chosen box is found, find its indexes 
        			if (boxes.get(i).get(j) == 0) {
        				// Illegal move
        				if(DEBUG)System.out.println("Illegal move branch.");//debug
        				return null;
        			} else if (boxes.get(i).get(j) == 1) {
        				// Regular move
        				if(DEBUG)System.out.println("Regular move branch.");//debug
        				...
        				// Losing move
        				if(DEBUG)System.out.println("Losing move branch.");//debug
        				return new Chomp(boxes);
        			} 
        		} else {
        			currentBox++;
